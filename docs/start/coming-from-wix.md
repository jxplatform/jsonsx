...
On Wix or Squarespace, your site lives inside your account: the editor, the files, and the hosting are one thing you rent. Jx separates them. You still build in a visual editor, clicking and typing and dragging and styling, but the site itself is a folder of ordinary files that you own, and putting it on the web means handing those files to a hosting service. This page explains what that actually means day to day, including the parts you take on yourself.

## You still get a visual editor

Jx Studio is a full visual builder: click any text and type right on the page, drag in headings, images, and sections, and style everything with panels for colors, spacing and fonts, while a live canvas shows the page at every screen size. Nothing in the [Studio documentation](/docs/studio) assumes you can code.
...
1. **Your computer** holds the project folder. That folder is the site: pages, images, settings, all as ordinary files Studio edits.
2. **A repository** is an online copy of that folder, with its full history. It's your backup and your publishing channel.
3. **A host** is the service that shows the site to visitors. Every time you sync changes to the repository, the host rebuilds and the new version goes live.

The day-to-day loop is: edit in Studio, click **Commit and sync**, and a minute later the site is updated. All of it happens through Studio's buttons, and you never type commands.
...
- **No single support desk.** If something on the host's side misbehaves, it's their documentation you'll read, because no one company is renting you the whole stack.

Editing also happens on the machine where Studio and your files are, rather than from any browser anywhere. The repository lets you set up a second machine, but it's a copy you open, not a website you log into.

## What you get back

The trade is real, and so is the payoff: nothing is locked in.

- A lapsed subscription can never take your site down or hold your content, because the files are on your machine and in your repository either way.
...
- **[Install Jx Studio](/docs/start/install)** to download the app for macOS, Windows, or Linux.
- **[Your first project](/docs/start/first-project)** goes from starter template to published site.
- **[A tour of Jx Studio](/docs/start/studio-tour)** helps you find your way around the editor.
