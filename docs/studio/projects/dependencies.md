...
- The **Packages** section of [Project settings](/docs/studio/projects/settings) shows each package's current version beside its newest version on npm — every package, Jx's own included, each measured against its own latest — with per-row update buttons and **Update all**.
