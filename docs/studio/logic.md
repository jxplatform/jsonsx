...
- **[Data](/docs/studio/logic/data)**: the Navigator rail's :kbd[⌘7]. _What does this page or component know, and what is it worth right now?_ Declare values, computed entries, data sources, and functions here, and read what each one resolved to as the page runs. Fetches wait for a **Refresh** or for preview rather than re-running on every edit.
...
1. Open a component and press :kbd[⌘7] for **Data**. Choose _+ Add… > Value_, name it `$count`, set its **Type** to `integer` and its **Default** to `0`. The component now knows a number.
...
- Reach for **State** first, because every other surface refers back to what you declare there.
