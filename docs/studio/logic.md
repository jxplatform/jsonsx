---
title: "Logic"
description: "Make pages interactive in Jx Studio: where the Data, Logic, Formula, and Code surfaces fit together, with a 60-second counter to see it working."
code:
...
- **[Data](/docs/studio/logic/data)**: the Navigator rail's :kbd[⌘6]. _What does this page or component know, and what is it worth right now?_ Declare values, computed entries, data sources, and functions here, and read what each one resolved to as the page runs. Fetches wait for a **Refresh** or for preview and do not re-run on every edit.
...
1. Open a component and press :kbd[⌘6] for **Data**. Choose _+ Add… > Value_, name it `$count`, set its **Type** to `integer` and its **Default** to `0`. The component now knows a number.
...
4. Pick **Preview** in the **View** control on the context bar and click the button. The number climbs, and in the **Data** panel (:kbd[⌘6]) you can watch `$count` change in real time.
...
- Declare your first values in the **[Data panel](/docs/studio/logic/data)**
