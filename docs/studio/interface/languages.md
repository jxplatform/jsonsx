...
:::doc-tip
:kbd[⌘K] → **Add Language** does the same write. It is the one language command available in a project that has none yet — which is exactly where you need it.
:::
...
:::doc-note
**Stale** means the default language's file is newer than the translation. A file your platform reports no timestamp for is shown as present, never stale — an absent timestamp isn't evidence of being behind.
:::
