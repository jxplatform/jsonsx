...
A data source is a state entry whose value comes from somewhere rather than being typed in: a URL, the browser's storage, a form, your project's content, a database table. You add one from the **[Data panel](/docs/studio/logic/data)**'s **+ Add…** picker, and from then on it behaves like any other value: bind it, compute from it, show it. Sources are reactive, so when the underlying data changes, everything built on it updates. The model behind that is **[Reactivity](/docs/framework/concepts/reactivity)**.

Each source's editor shows just the fields that source needs.

## Request: fetch from a URL

**Fetch from a URL** loads data over HTTP, most typically a JSON API.
...
- **Timing**: `client` fetches in the visitor's browser; `server` runs the request on the server instead, which is useful when an API can't be called from a browser.
...
Everything above reads from the browser, the network, or your project's files. With the connector extension enabled, your **[data tables](/docs/studio/data)** join the picker too. They hold the data that appears and changes while the site runs. Each of these sources starts with a **table**, the name of a table your project defines:
