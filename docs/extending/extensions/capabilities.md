...
Capabilities are how a class's declarations turn into behavior. They are declared in the descriptor's `$defs.methods` using well-known `role` values, and they are all `scope: "static"`, so hosts call them on the implementation class without constructing an instance. The instance `resolve()` method remains the runtime's on-demand access path; capabilities serve the build, the servers, and Studio.
...
- `jx preview` needs nothing, because it serves `dist/`, where the files already are.
