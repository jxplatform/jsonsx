...
Ship compiled output to production and the eval requirement disappears, which is the main reason the compiler exists.

## The interpreter needs `'unsafe-eval'`

The interpreting runtime, meaning the dev server, the Studio canvas, and `@jxsuite/runtime` used directly as a library, compiles `${}` templates and inline `body` functions with `new Function` on the fly. Any page that hosts the interpreter must allow `'unsafe-eval'` in its CSP.
...
**Nothing Jx ships writes `innerHTML`.** The runtime and the compiled element modules clear elements with `replaceChildren()` instead, which is identical behavior and not a DOM injection sink. That is a smaller surface in every site you build, whether or not any policy is enforcing.
...
This is why the canvas and the shell keep separate policies permanently, and it works only because the canvas is a page with its own response: a frame loaded over `http` gets its own policy, while a `srcdoc` frame inherits its parent's.
...
If you compile documents you did not author, such as user-submitted content merged into the tree, sanitize or escape `${` sequences first.
