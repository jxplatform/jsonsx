...
The index that powers the site's own search box, emitted by the [search extension](/docs/framework/site/search) during the build. A `documents` array wrapped in an envelope of `version`, `engine` (`"minisearch"`), the indexed `fields`, and per-field `boost` weights. Each document is one page or one heading-level section within a page, carrying `id`, `collection`, `slug`, `url`, `title`, `description`, `heading`, and `text`. A collection that keeps one directory per language adds `locale`, and then the `id` carries the tag too (`docs:fr-CA:start/install`), because two translations share an entry id.
