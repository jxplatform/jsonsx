...
Collections don't need this. Two translations of one entry [share an id](#content-in-one-directory-per-locale), so their URLs are already parallel — and a key on a `[slug]` template would claim every entry it expands.
