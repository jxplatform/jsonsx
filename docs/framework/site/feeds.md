...
`url` must be set in `project.json`, because a feed's entries are identified by absolute URL and there is nothing to generate without one.
...
Every page advertises all of them, each tagged with `hreflang`, because the discovery links are written before the build knows which language the page is in, so they name every language and let the reader's client choose.
