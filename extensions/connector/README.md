...
`Request` fetches against `/_jx/data`, actions become inline `Function` handlers, so no extension
...
extension's hooks on the shared server context, so absent auth means 401. Defaults: read `public`,
...
`schemas.fields` fragment. That manifest convention (a fragment whose `$defs` members join the
