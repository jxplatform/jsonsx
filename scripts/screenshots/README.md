...
No browser is downloaded — that is what makes the runner work on NixOS and plain CI alike.
...
`{ "region": "<id>" }` — the id resolves to an element with a non-empty box. `{ "state": {…} }` — a
...
1. **`probe.idle()`** — the app's own account: queued lit renders, pending panel-scheduler frames,
   ...
   accepted.
2. **The runner's quiescence** — outstanding network requests across every frame, running Web
...
**Lane 1 — `scripts/check-shot-contract.ts`, every PR, no browser, seconds.** Fails when a `cmd` id
...
**Lane 2 — `.github/workflows/screenshots.yml`.** Required on any PR touching
...
- A shot may carry `"status": {"state": "quarantined", "reason": "…", "since": "<sha>"}` — a shot the
...
- Markdown pages show inline-edit placeholders in **design** view — use `preview` for clean content
...
- Every phase that renames a command id, a panel id or a region id fixes the manifest **in the same
  ...
  with `probe.idle()` — the reflex when a shot times out will be to re-add a sleep, and that is how
