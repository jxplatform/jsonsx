...
- The `action_required` state those runs sit in is a repository Actions policy, not a termination condition. Approve one and the lane re-captures its own output and pushes again; the button is the crank handle, not the brake.
