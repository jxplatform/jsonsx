...
- `@jxsuite/protocol/types`: the request/response shapes (`DirEntry`,
  ...
  Environment-agnostic, with no DOM and no node, so the types are importable
  in browsers, Bun, and Cloudflare Workers alike.
...
- **`cf/proxy`** is an allowlisted Cloudflare API passthrough (accounts and
  ...
  and must never be persisted by stateless implementations. On the dev server
  the credential is a header-borne user token, and on cloud platforms a
  vaulted OAuth token.
