...
describe("mergeHead", () => {
  test("auto-injects charset and viewport defaults", () => {
    const result = /** @type {any []} */ (mergeHead());
  ...
  test("injects title from context", () => {
    const result = /** @type {any []} */ (mergeHead([], [], [], { title: "My Page" }));
    const titleEntry = result.find((e) => e.tagName === "title");
    expect(titleEntry).toBeDefined();
    expect(/** @type {any} */ (titleEntry).children).toEqual(["My Page"]);
  });

  test("uses siteName as fallback title", () => {
    const result = /** @type {any []} */ (mergeHead([], [], [], { siteName: "Jx Site" }));
    const titleEntry = result.find((e) => e.tagName === "title");
    expect(/** @type {any} */ (titleEntry).children).toEqual(["Jx Site"]);
  });
  ...
  test("later layers override earlier ones (page > layout > site)", () => {
    ...
    const result = /** @type {any []} */ (mergeHead(site, layout, page));
    const desc = result.find((e) => e.attributes?.name === "description");
    expect(/** @type {any} */ (desc).attributes.content).toBe("Page desc");
  });
  ...
  test("deduplicates <meta charset>", () => {
    ...
    const result = /** @type {any []} */ (mergeHead(site, [], page));
    const charsets = result.filter((e) => e.attributes?.charset);
    expect(charsets).toHaveLength(1);
    expect(/** @type {any} */ (charsets[0]).attributes.charset).toBe("utf-16");
  });
  ...
  test("deduplicates <meta property> (Open Graph)", () => {
    ...
    expect(/** @type {any} */ (og[0]).attributes.content).toBe("Page Title");
  });
  ...
  test("adds canonical URL when pageUrl and siteUrl provided", () => {
    ...
    expect(/** @type {any} */ (canonical).attributes.href).toBe("https://example.com/about");
  });

  test("does not add canonical without both URLs", () => {
    const result = /** @type {any []} */ (mergeHead([], [], [], { pageUrl: "/about" }));
    const canonical = result.find((e) => e.tagName === "link" && e.attributes?.rel === "canonical");
    expect(canonical).toBeUndefined();
  });

  test("respects custom charset from context", () => {
    const result = /** @type {any []} */ (mergeHead([], [], [], { charset: "utf-16" }));
    const charset = result.find((e) => e.attributes?.charset);
    expect(/** @type {any} */ (charset).attributes.charset).toBe("utf-16");
  });
...
describe("renderHead", () => {
  ...
  test("handles string entries", () => {
    const html = renderHead([/** @type {any} */ ("<!-- custom -->")]);
