...
import { buildImageHandlerSource, buildImageWorkerRoute } from "./compile-image-endpoint.ts";
 ...
 * @param {ImageConfig | null} [opts.images] - When set, embed the Cloudflare Images binding
 *   endpoint at `/_jx/image` (images.service: "cloudflare")
...
export function compileSiteServer(
  ...
  const { baseUrl = "/_jx/server", adapter = null, images = null } = opts;
  ...
  // The image route must register before the cloudflare-workers ASSETS catch-all below.
  const imageRoute = images ? buildImageWorkerRoute() : "";
 ...
 */
export function compilePagesFunctions(
  entries: { exportName: string; src: string }[],
  opts: {
    baseUrl?: string;
  } = {},
