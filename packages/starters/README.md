...
import registry from "@jxsuite/starters/registry.json" with { type: "json" };
...
that does not exist. The package publishes raw TypeScript — `exports["."]` is
`./index.ts`, there is no build step, and the module imports only `node:fs` and
`node:path`.
...
different count (digits and number words alike). It does not scan this file, so
...
sites/restaurant/
  ...
  images.json            # authoring-only Pexels manifest; stripped from clones
