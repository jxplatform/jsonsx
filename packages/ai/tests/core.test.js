...
describe("ToolRegistry", () => {
  it("registers and lists tools", () => {
    ...
    registry.register(
      createToolDefinition({
        ...
        execute: (args) => toolSuccess(`Hello, ${args.name}!`),
      }),
    );
  ...
  it("lists tools in LLM format", () => {
    ...
    expect(llmTools[0].function.name).toBe("greet");
  });
  ...
  it("executes tools and returns results", async () => {
    ...
    registry.register(
      createToolDefinition({
        ...
        execute: (args) => toolSuccess(args.a + args.b, `Sum: ${args.a + args.b}`),
      }),
    );
  ...
  it("respects strict: false (skips validation)", async () => {
    ...
    registry.register(
      createToolDefinition({
        ...
        strict: false,
        execute: (args) => toolSuccess(args.x || "default"),
      }),
    );
...
describe("StreamingClient", () => {
  ...
  it("Anthropic client stub returns error event", async () => {
    ...
    expect(events[0].message).toContain("not yet implemented");
  });
});
