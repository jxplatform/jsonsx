...
Every implementation yields the same discriminated union, whose type strings are also published as
...
- **`createOpenAIStreamingClient({ baseUrl, apiKey, model = "gpt-4o", temperature })`** POSTs to
  ...
  when defined**, because reasoning models (GPT-5.x, o-series) reject a custom temperature. That is a
...
its mutators `Object.assign`ed onto the same proxy, so the returned value is both the state and the
