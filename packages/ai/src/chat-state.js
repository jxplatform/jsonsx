...
/**
 * @typedef {"idle" | "streaming" | "error"} ChatState
 *
 ...
 * }} ToolCallRecord
 *
 * @typedef {import("./tools.js").ToolResult} ToolResult
 *
 * @typedef {{
 *   messages: Message[];
 *   status: ChatState;
 ...
 * }}
 */
export function createChatState(opts = {}) {
  const model = opts.model || "gpt-4o";

  const store = /** @type {ChatStore} */ (
    reactive({
      messages: [],
      status: "idle",
      streamingContent: "",
   ...
   */
  function pushToolResultMessage(toolCallId, content) {
    store.messages.push(
      /** @type {Message} */ ({
        id: uid(),
        role: "tool",
   ...
   * @param {string} name
   */
  function appendToolCallStart(id, name) {
    if (store.status !== "streaming") {
      return;
    }
    const tc = /** @type {ToolCallRecord} */ ({
      id,
      name,
      arguments: "",
      result: null,
   ...
   * @param {string} args
   */
  function appendToolCallDelta(id, args) {
   ...
   * @param {string} _id
   */
  function appendToolCallEnd(_id) {
   ...
   */
  function finishStream(_stopReason) {
    store.status = "idle";
    store.streamingContent = "";
    store.pendingToolCalls = [];
   ...
   */
  function setContextWarning(warning) {
