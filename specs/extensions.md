...
**self-contained bundle**: the core schema and every enabled extension's
fragments are embedded under `$defs` keyed by their canonical `$id`s, and all
composition refs are those canonical URIs. No committed entry document
carries a relative-path `$ref` — relative `./node_modules/...` refs resolve
only through Node module resolution, which editors do not perform (hoisted
workspaces leave in-repo projects without their own `node_modules`), so the
bundled form is the ONE artifact with ONE resolution behavior for ajv
`jx validate`, VS Code's JSON language service, and the studio's Monaco
alike. `jx dev` does not run it on startup (it builds the site); live
regeneration happens **on demand** instead: the dev server's
...
(`@jxsuite/schema/schema.json`) embedded the same way (root `$ref` to its
canonical `$id`, `https://jxsuite.com/schema/v1`), re-embedding the paths
union resource (`https://jxsuite.com/schema/document/paths/v2`) as the union
of extension-contributed `$paths` shapes.

The emitter composes with project-relative fragment paths as an intermediate
form only; the committed output is always run through the generic bundler
(§5.4) first. A resource whose `$id` is already present in the compound
document is never embedded twice, so re-bundling a committed entry document
is a no-op.
...
- Any compliant validator resolves the committed entry documents **offline
  with no file access at all**: they are self-contained bundles (§5.2), so
  every `$ref` is either a local pointer or a canonical URI resolving to an
  embedded resource's `$id`. Editors (VS Code's JSON language service,
  Monaco) index embedded `$id`s and need no `node_modules`, no network, and
