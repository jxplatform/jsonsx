...
| [Trusted Types](https://www.w3.org/TR/trusted-types/)                                     | **Subset**    | §21.5    | packages/studio/src/services/trusted-types.ts, packages/server/src/studio-csp.ts, packages/studio/src/services/csp-report.ts, packages/server/tests/studio-csp.test.ts | `gap:trusted-types` The policy exists and refuses: `createHTML` asserts its input is sanitized and throws naming what it found, `createScript`/`createScriptURL` throw outright, and the runtime's four `innerHTML = ""` writes became `replaceChildren()`. **Observed, not enforced** — the shell is served `Content-Security-Policy-Report-Only: require-trusted-types-for 'script'` and files each violation as a Problem, which is the stage §21.5 mandates before enforcing. Enforcement stays out because under this standard `eval` and `new Function` are gated too: the canvas interpreter needs both permanently, and the run showed the shell evaluates as well. |
...
> Trusted Types policy guards the shell's own injection sink, and the **observation stage now ships**
> — both servers send the shell `Content-Security-Policy-Report-Only: require-trusted-types-for
'script'` and the shell files each violation as a Problem (§21.5). What is absent is
> **enforcement**, and the run is why it stays absent: the shell turns out to evaluate strings (the
> interpreter runs there for Library preview and render-check) and to carry dependency-owned
> `innerHTML` sinks that no policy of ours can reach. Each needs a decision, not more policy code.
> See §18.
...
- **Jx's own injection sinks are handled.** The four `innerHTML = ""` writes in `@jxsuite/runtime`
  became `replaceChildren()` — identical semantics, four fewer sinks, and no policy needed to clear
  an element. The assistant's markdown goes through a policy that **asserts** its input is sanitized
  and throws naming what it found. A `createHTML` that returned its input unchanged would satisfy
  the API and defend nothing.
- **The shell's remaining sinks are its dependencies', and they cannot be routed through the
  policy.** This paragraph previously said the assistant's markdown was the only one left. The
  report-only run's first boot disproved it: `<sp-theme>` is in `index.html`, and Spectrum's
  `Theme.template` does `templateElement.innerHTML = "<slot></slot>"` before any author has clicked
  anything. Tabulator and Monaco carry their own. None is reachable from `jx-studio` — a
  third-party library writes to its own sink, and the only lever over it is a `trusted-types`
  allow-list, which admits the pass-through this section rejects.
- **`createScript` and `createScriptURL` refuse, and the shell has sinks for both.** This section
  also claimed nothing in the shell builds either from a string. Three things do: Monaco's worker
  override builds a URL and passes it to `new Worker` (`monaco-setup.ts`); `jx-validate.ts` compiles
  schemas with Ajv, whose codegen is `new Function`; and the interpreter itself runs **in the shell
  document** for Library preview, render-check and component preview, reaching `new Function` in
  `runtime.ts`. §21.3 says the canvas evaluates permanently; it did not say the shell does too, and
  it does. The refusals stay — a permissive `createScript` would re-permit evaluation for the whole
  shell to buy nothing — but "the shell is nearly ready to enforce" was not true.
- **The observation stage ships.** Both servers send the shell
  `Content-Security-Policy-Report-Only: require-trusted-types-for 'script'`
  (`packages/server/src/studio-csp.ts`), and the shell files each `SecurityPolicyViolationEvent` as
  a Problem (`packages/studio/src/services/csp-report.ts`, §16's channel, deduped per call site).
  Four properties of the standard decided the shape, each verified in a browser rather than read:

  | Verified                                                                                   | Consequence                                                                                                                              |
  | ------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------- |
  | A report-only policy in `<meta>` is ignored entirely (CSP3 §3.3)                           | The header comes from the servers; `index.html` gets no `<meta>`                                                                         |
  | `require-trusted-types-for 'script'` reports `eval` and `new Function`, not only DOM sinks | The canvas is excluded by path, not by hope                                                                                              |
  | With no `trusted-types` allow-list, policy creation raises no violation                    | One directive token; the dozen third-party policies produce no noise, and no pass-through is blessed                                     |
...
- **Enforcement is still not scheduled, and now has a shorter list of unknowns.** The order was: ship
  the policy, observe under `Content-Security-Policy-Report-Only`, then enforce. The first two are
  done. Enforcement waits on a decision about each observed violation — every one so far belongs to
  a dependency or to the interpreter, and neither is fixed by writing more policy code.
