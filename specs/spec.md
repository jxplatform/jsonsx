...
| [WHATWG HTML](https://html.spec.whatwg.org/)                                              | **Subset**    | §16.6    | packages/compiler/src/targets/compile-element.ts, packages/compiler/tests/no-shadow-dom.test.ts | `gap:declarative-shadow-dom` Custom elements are defined and upgraded as the standard describes, and then render into the **light DOM**: no `attachShadow`, no `shadowrootmode`, no `adoptedStyleSheets`, asserted by a committed test over both the emitted module and the emitter sources. `<slot>` is emulated by splicing saved children, so distribution semantics differ from real slotting. The opt-in — `$shadow` on a component, `defaults.shadow` on a project — is designed and unbuilt; light DOM stays the default either way. |
...
_Jx Specification v0.4.30-draft — subject to revision_
