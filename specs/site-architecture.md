...
> **Status: Partial.** §13.1, §13.2 and §13.4 ship: tags are validated and canonicalized, a route's
> prefix decides its locale, and each page carries the `lang` and `dir` that follow. What does not
> is discovery and negotiation — no `hreflang` alternates in `<head>` or the sitemap, no
> `Accept-Language`, and §13.3's `{locale}` collections are still unread. A translated site is
> therefore correct to a reader and invisible to a search engine.
>
