...
2.  **Manifest check** — `bun scripts/check-coverage-manifest.ts <workspace-dir>` (e.g. `packages/schema`, `extensions/auth`) fails if any `src/**/*.ts` file never appears in `coverage/lcov.info` (Bun only counts files imported during the run, so a brand-new untested file is otherwise invisible to thresholds). Type-only files are allowlisted inside the script.
    ...
    **The one instance of that defect has a known shape, and it is not about CI.** Bun 1.4.0 loses a source file's coverage record when one test file fires **two or more un-awaited `import()`s** of it, from separate tests, and another test file running **later** imports it for real — the later file's tests pass against the real module throughout, and the module is simply absent from both the lcov and the printed table. One fire is not enough; two is. `bun test` ignores the order of its file arguments and runs whatever `readdir` hands it, so the same clone flips between recording and not when either file is rewritten, which is what made it look environmental for weeks. Practical rule when you write a test that invokes a command whose `run` lazily imports its implementation: **double the implementation with `mock.module()`**, as `commands-defaults.test.ts`, `context-menu.test.ts` and `settings-key-scope.test.ts` all do. It is what gives the test a witness — an un-awaited import settles after `run()` returns, so without a double there is nothing to assert against — and it keeps the coverage record. Reported upstream; the full rule and its five-file reproduction are in the header of `scripts/check-coverage-manifest.ts`.
