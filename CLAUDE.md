...
- **Gates**: `bun run docs:check` (associations), `bun run docs:verify` (generated-page drift, clean tree only), `bun run docs:standards` (the alignment tables), `bun run docs:markdown` (visual-editor escapes; wrapping once `--no-wrap` goes) — all must stay green.
