...
**A ban can rewrite a claim out of true.** The `neg-parallel` rule fires on the shape `doesn't just X but Y`, and the obvious fix inverts it. `studio/ai.md` said the assistant `doesn't just talk about your project but works on it`, and the rewrite made it work on your project rather than talk about it, which denies the first half and contradicts the same page twelve lines down. The shape asserts BOTH halves. State both, or state the stronger one; never keep the shape and drop a half. The verification pass caught this and no gate could have, because a flipped claim moves no identifier and no number.
